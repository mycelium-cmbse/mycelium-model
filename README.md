...
- Every release now runs `.github/scripts/check-nuspec-dependencies.py` against the packed `.nupkg` before `dotnet nuget push`, failing the run if any nuspec `<dependency>` names a version that isn't on NuGet.org. The ordering constraint in the next bullet is therefore enforced, not merely documented.
...
`.github/scripts/stamp-xmi-version.py` writes three `mofext:Tag` elements onto the model's top-level `uml:Package`, declaring `xmlns:mofext="http://www.omg.org/spec/MOF/20131001"` on the root `<xmi:XMI>` if it isn't there already:
